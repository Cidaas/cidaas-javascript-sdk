...
export interface IInitVerificationAuthenticationRequestEntity {
  ...
  request_id: string; // application request id, generated by oauth call

  type?: string // medium type
  medium_id?: string; // id for medium type (email, sms)

  push_id?: string; // is a firebase token, used to validate for mobile apps
  device_id?: string; // device unique id to do authentication (e.g. smartphone)

  single_factor_auth?: boolean; // ?? TODO: Clarify with Ganesh what it does
  single_factor_sub_ref?: string; // ?? TODO: Clarify with Ganesh what it does
